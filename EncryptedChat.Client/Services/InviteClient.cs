...
public sealed class InviteClient(HttpClient http)
{
    ...
    {
        var res = await _http.PostAsync($"api/Team/{teamId}/invites", null);
    ...
    {
        var res = await _http.GetAsync($"api/Team/{teamId}/invites");
        return res.IsSuccessStatusCode ? (await res.Content.ReadFromJsonAsync<List<InviteListItem>>() ?? new()) : new();
    }
    ...
    {
        var res = await _http.GetAsync($"api/Team/invite/{Uri.EscapeDataString(token)}");
    ...
    {
        var res = await _http.PostAsync($"api/Team/invite/{Uri.EscapeDataString(token)}/join", null);
